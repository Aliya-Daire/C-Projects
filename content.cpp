...
// Define all three classes in single .cpp file

// To do - Complete this function
Content::Content(int id, std::string name, int nr, int ts, int rating)
{ // MOVIES/SERIES users can watch - all content has the following data:
  this->id_ = id; // (unique) integer ID
  this->name_ = name; // a NAME (by which it can be searched)
  this->numReviews_ = nr; // count of its REVIEWS
  this->totalStars_ = ts; // total number of STARS given from all the reviews
  this->rating_ = rating; // parental control RATING (0-4)
}
...
{ // Get the average number of stars over all the reviews
  if (numReviews_ == 0)
  {
    return 0;
  }
  return static_cast<double>(totalStars_)/static_cast<double>(numReviews_); // return double review average
}
